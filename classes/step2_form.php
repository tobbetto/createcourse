 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class local_createcourse_step2_form extends local_createcourse_base_form {
    ...
    public function definition () {
        ...
        // Course options.
        /*$mform->addElement('header', 'courseoptionshdr', get_string('courseprocess', 'local_createcourse'));
        $mform->setExpanded('courseoptionshdr', true);

        $mform->addElement('text', 'options[shortnametemplate]', get_string('shortnametemplate', 'local_createcourse'),
            'maxlength="100" size="20"');
        $mform->setType('options[shortnametemplate]', PARAM_RAW);
        $mform->addHelpButton('options[shortnametemplate]', 'shortnametemplate', 'local_createcourse');
        $mform->hideIf('options[shortnametemplate]', 'options[mode]', 'eq', local_createcourse_processor::MODE_CREATE_OR_UPDATE);
        $mform->hideIf('options[shortnametemplate]', 'options[mode]', 'eq', local_createcourse_processor::MODE_UPDATE_ONLY); */

        // Restore file is not in the array options on purpose, because formslib can't handle it!
        $contextid = $this->_customdata['contextid'];
        /*$mform->addElement('hidden', 'contextid', $contextid);
        $mform->setType('contextid', PARAM_INT);
        $mform->addElement('filepicker', 'restorefile', get_string('templatefile', 'local_createcourse'));
        $mform->addHelpButton('restorefile', 'templatefile', 'local_createcourse');

        $mform->addElement('text', 'options[templatecourse]', get_string('coursetemplatename', 'local_createcourse'));
        $mform->setType('options[templatecourse]', PARAM_TEXT);
        $mform->addHelpButton('options[templatecourse]', 'coursetemplatename', 'local_createcourse');

        $mform->addElement('selectyesno', 'options[reset]', get_string('reset', 'local_createcourse'));
        $mform->setDefault('options[reset]', 0);
        $mform->hideIf('options[reset]', 'options[mode]', 'eq', local_createcourse_processor::MODE_CREATE_NEW);
        $mform->hideIf('options[reset]', 'options[mode]', 'eq', local_createcourse_processor::MODE_CREATE_ALL);
        $mform->disabledIf('options[reset]', 'options[allowresets]', 'eq', 0);
        $mform->addHelpButton('options[reset]', 'reset', 'local_createcourse'); */

        // Default values.
        /*$mform->addElement('header', 'defaultheader', get_string('defaultvalues', 'local_createcourse'));
...
*/
        ...
        $handler->instance_form_definition($mform, 0, 'defaultvaluescustomfieldcategory', 'local_createcourse');
        ...
        $this->add_action_buttons(true, get_string('uploadcourses', 'local_createcourse'));
    ...
    public function add_action_buttons($cancel = true, $submitlabel = null) {
        $mform =& $this->_form;
        $buttonarray = array();
        $buttonarray[] = &$mform->createElement('submit', 'showpreview', get_string('preview', 'local_createcourse'));
