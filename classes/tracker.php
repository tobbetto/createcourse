 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class local_createcourse_tracker {
    ...
    public function results($total, $created, $updated, $deleted, $errors) {
        ...
        $message = array(
            get_string('coursestotal', 'local_createcourse', $total),
            get_string('coursescreated', 'local_createcourse', $created),
            get_string('coursesupdated', 'local_createcourse', $updated),
            get_string('coursesdeleted', 'local_createcourse', $deleted),
            get_string('courseserrors', 'local_createcourse', $errors)
        );
    ...
    public function output($line, $outcome, $status, $data) {
        ...
        if ($this->outputmode == self::OUTPUT_PLAIN) {
            ...
            echo html_writer::tag('td', isset($data['startdate']) ? $data['startdate'] : '', array('class' => 'c' . $ci++));
            echo html_writer::tag('td', isset($data['shortname']) ? $data['shortname'] : '', array('class' => 'c' . $ci++));
            echo html_writer::tag('td', isset($data['fullname']) ? '<a href="' . new moodle_url('/course/view.php', array('id' => $data['id'])) . '">' . $data['fullname'] . '</a>': '', array('class' => 'c' . $ci++));
            echo html_writer::tag('td', isset($data['category']) ? '<a href="' . new moodle_url('/course/management.php', array('categoryid' => $data['category'])) . '">' . $data['scope'] . ' / ' . $data['mode'] . ' / ' . $data['applicative'] . '</a>' : '', array('class' => 'c' . $ci++));
            echo html_writer::tag('td', isset($data['teachers']) ? $data['teachers'] : '', array('class' => 'c' . $ci++));
            echo html_writer::tag('td', $status, array('class' => 'c' . $ci++));
            echo html_writer::end_tag('tr');
        }
    }
   // '<a href="' . new moodle_url('/course/view.php', array('id' => $course->id)) . '">' .   . '</a>'
    /**
     * Start the output.
     *
     * @return void
    ...
    public function start() {
        ...
        if ($this->outputmode == self::OUTPUT_PLAIN) {
            ...
            echo html_writer::start_tag('table', array('class' => 'generaltable boxaligncenter flexible-wrap',
                'summary' => get_string('uploadcoursesresult', 'local_createcourse')));
            echo html_writer::start_tag('tr', array('class' => 'heading r' . $this->rownb));
            echo html_writer::tag('th', get_string('csvline', 'local_createcourse'),
                array('class' => 'c' . $ci++, 'scope' => 'col'));
            echo html_writer::tag('th', get_string('result', 'local_createcourse'), array('class' => 'c' . $ci++, 'scope' => 'col'));
            echo html_writer::tag('th', get_string('startdate'), array('class' => 'c' . $ci++, 'scope' => 'col'));
            ...
            echo html_writer::tag('th', get_string('teachers'), array('class' => 'c' . $ci++, 'scope' => 'col'));
