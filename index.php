...
admin_externalpage_setup('localcreatecourse');

$importid         = optional_param('importid', '', PARAM_INT);
$previewrows = optional_param('previewrows', 10, PARAM_INT);

$returnurl = new moodle_url('/local/createcourse/index.php');

if (empty($importid)) {
    $mform1 = new local_createcourse_step1_form();
    if ($form1data = $mform1->get_data()) {
        ...
        unset($content);
        if ($readcount === false) {
            print_error('csvfileerror', 'local_createcourse', $returnurl, $cir->get_error());
        } else if ($readcount == 0) {
        ...
        echo get_string('fecasita', 'local_createcourse') . ' ' . $OUTPUT->heading_with_help(get_string('uploadcourses', 'local_createcourse'), 'uploadcourses', 'local_createcourse');
        $mform1->display();
        echo $OUTPUT->footer();
        die();
    }
...
$mform2 = new local_createcourse_step2_form(null, array('contextid' => $context->id, 'columns' => $cir->get_columns(),
    'data' => $data));
...
if ($form2data = $mform2->is_cancelled()) {
    ...
    $customfields = local_createcourse_helper::get_custom_course_field_names();
    ...
    $processor = new local_createcourse_processor($cir, $options, $defaults);

    echo $OUTPUT->header();
    if (isset($form2data->showpreview)) {
        echo get_string('fecasita', 'local_createcourse') . ' ' . $OUTPUT->heading(get_string('uploadcoursespreview', 'local_createcourse'));
        $processor->preview($previewrows, new local_createcourse_tracker(local_createcourse_tracker::OUTPUT_HTML));
        $mform2->display();
    } else {
        echo get_string('fecasita', 'local_createcourse') . ' ' . $OUTPUT->heading(get_string('uploadcoursesresult', 'local_createcourse'));
        $processor->execute(new local_createcourse_tracker(local_createcourse_tracker::OUTPUT_HTML));
    ...
    if (!empty($form1data)) {
        ...
        $options = array('mode' => local_createcourse_processor::MODE_CREATE_NEW);
    }
    $processor = new local_createcourse_processor($cir, $options, array());
    echo $OUTPUT->header();
    echo get_string('fecasita', 'local_createcourse') . ' ' . $OUTPUT->heading(get_string('uploadcoursespreview', 'local_createcourse'));
    $processor->preview($previewrows, new local_createcourse_tracker(local_createcourse_tracker::OUTPUT_HTML));
    $mform2->display();
