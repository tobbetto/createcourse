...
Example:
\$sudo -u www-data /usr/bin/php local/createcourse/cli/createcourse.php --mode=createnew \\
...
$modes = array(
    'createnew' => local_createcourse_processor::MODE_CREATE_NEW,
    'createall' => local_createcourse_processor::MODE_CREATE_ALL,
    'createorupdate' => local_createcourse_processor::MODE_CREATE_OR_UPDATE,
    'update' => local_createcourse_processor::MODE_UPDATE_ONLY
);
if (!isset($options['mode']) || !isset($modes[$options['mode']])) {
    echo get_string('invalidmode', 'local_createcourse')."\n";
    echo $help;
    die();
}
...
$updatemodes = array(
    'nothing' => local_createcourse_processor::UPDATE_NOTHING,
    'dataonly' => local_createcourse_processor::UPDATE_ALL_WITH_DATA_ONLY,
    'dataordefaults' => local_createcourse_processor::UPDATE_ALL_WITH_DATA_OR_DEFAUTLS,
    'missingonly' => local_createcourse_processor::UPDATE_MISSING_WITH_DATA_OR_DEFAUTLS
);
if (($processoroptions['mode'] === local_createcourse_processor::MODE_CREATE_OR_UPDATE ||
        $processoroptions['mode'] === local_createcourse_processor::MODE_UPDATE_ONLY)
        && (!isset($options['updatemode']) || !isset($updatemodes[$options['updatemode']]))) {
    echo get_string('invalideupdatemode', 'local_createcourse')."\n";
    echo $help;
    die();
}
...
if (!file_exists($options['file'])) {
    echo get_string('invalidcsvfile', 'local_createcourse')."\n";
    echo $help;
    die();
}
...
if (!isset($encodings[$options['encoding']])) {
    echo get_string('invalidencoding', 'local_createcourse')."\n";
    echo $help;
    die();
}
...
if ($options['restorefile'] && !file_exists($options['restorefile'])) {
    echo get_string('invalidrestorefile', 'local_createcourse')."\n";
    echo $help;
    die();
}
...
unset($content);
if ($readcount === false) {
    print_error('csvfileerror', 'local_createcourse', '', $cir->get_error());
} else if ($readcount == 0) {
    print_error('csvemptyfile', 'error', '', $cir->get_error());
}
$processor = new local_createcourse_processor($cir, $processoroptions, $defaults);
$processor->execute(new local_createcourse_tracker(local_createcourse_tracker::OUTPUT_PLAIN));
